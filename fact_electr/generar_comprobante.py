...
def generar_comprobante(tipo_de_comprobante: int, tipo_documento: str, numero_documento: str, pedido_id: int):
  """ Sirve para generar un comprobante electronico ya sea factura, boleta o notas en base a un pedido 
    https://docs.google.com/document/d/1QWWSILBbjd4MDkJl7vCkL2RZvkPh0IC7Wa67BvoYIhA/edit
  """
  ...
  #Froma sin hacer una segundacondicion
  # try:
  ...
  if pedido.total > 700 and tipo_documento == '-':
  ...
  if tipo_de_comprobante == 1:
    # esta serie de deberia guardar en la bd en una tabla de series para que cuando el contador desee cambiar de serie se modifique en esa tabla
    serie = 'FFF1'
    tipo = 'FACTURA'
  ...
  if tipo_documento is None:
    ...
    if tipo_documento == 'RUC':
      respuesta_apiperu = requests.get('https://apiperu.dev/api/ruc/'+numero_documento,headers={'Authorization':'Bearer '+environ.get('APIPERU_TOKEN')})
      ...
      respuesta_apiperu = requests.get('https://apiperu.dev/api/dni/'+numero_documento,headers={'Authorization':'Bearer '+environ.get('APIPERU_TOKEN')})
  ...
  cliente_email= 'carlos.ademir_66@hotmail.com'
  ...
  for detalle in detalle_pedido:    
    ...
    stock:Stock =detalle.stock
    # Extraigoel plaoto de ese stock
